...
const dom = {
  ...
  boxdiv: document.querySelector(".search_box"),
...
function init() {
  ...
  // 修改表单提交行为
  dom.boxdiv.addEventListener("submit", function(e) {
    e.preventDefault();
    const searchText = dom.input.value.trim();
    if (searchText) {
      // 直接使用配置的action URL，不需要paramKey
      window.open(dom.boxdiv.action + encodeURIComponent(searchText), '_blank');
    }
  });
}
...
function changeSearch(index) {
  ...
  // 更新表单属性
  dom.boxdiv.action = searchEngines[index].action;
}
